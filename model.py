...
# LOADING DATA
# vocab = pickle.load(open('saved_models/vocab_coco (1).file', 'rb'))
vocab = pickle.load(open('saved_models/version_7/vocab_coco (1).file', 'rb'))
...
def load_image_from_path(img):
    ...
    # img = tf.io.decode_jpeg(img.astype(np.float32), channels=3)
...
def get_caption_model():
    ...
    caption_model.call((sample_x, sample_y), training=False)
    ...
    inputs = [(None, None, None, 3), training, decoder_input]
    print(inputs)
    print(inputs[0])
    caption_model.build(input_shape=inputs)

    
    # caption_model.load_weights('saved_models/model (2).h5')
    caption_model.load_weights('saved_models/version_7/model (2).h5')
